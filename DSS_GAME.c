...
// Tempo maximo de duração de efeito sonoro
...
// A textura apresentada
SDL_Texture* g_texture = NULL;
SDL_Texture* g_pele = NULL;
...
void load_pele( char* l_Path )
{
        ...
        {
                // Gera textura atraves da surface
        g_pele = SDL_CreateTextureFromSurface( g_renderer, l_loadedSurface );
                if( g_texture == NULL )
                {
                        printf( "Unable to create texture from %s! SDL Error: %s\n", l_Path, SDL_GetError() );
                        g_quitGame = 1;
...
int gameplay_logic( int argc, char *argv[] )
{
	...
	load_pele( "sprites/sombra100-60.png" );
